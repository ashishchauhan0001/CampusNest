...
const Property = () => {
  ...
  const handleBookingRequest = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/tenant/gettenant/${id}`
      );
      ...
      if (response.status === 200) {
        ...
        const postResponse = await axios.post(
          "http://localhost:3000/api/request/addrequest",
          data
        );

        if (postResponse.status === 201) {
          console.log(
            "Data successfully sent and stored in the database:",
            postResponse.data
          );
        } else {
          console.error(
        ...
        console.error(
          "Failed to fetch tenant profile data:",
          response.statusText
        );
      }
    } catch (error) {
